...
// EPIC EDIT start -- nick.shin 2020-06-15 -- 1.39.18
...
//#endif
    if ( target == 0 ) { target = 'canvas'; }              // assume "canvas"
    else {
      var savetarget = target;
      target = __maybeCStringToJsString(target);
      if (target == 0) target = savetarget;                // original target was valid
      if (target == "#window") return window;              // might need to check for other "#id's" but, so far, this is the only one that crashes on querySelector()
    }
    var domElement = specialHTMLTargets[target];           // seems to also be [object Window] (i.e. target == 2)
    if (domElement != null) return domElement;
#if ENVIRONMENT_MAY_BE_WORKER || ENVIRONMENT_MAY_BE_NODE
    if (typeof document === 'undefined') return undefined; // either worker thread or doc not yet ready -- in any case, ignore...
#endif
    domElement = document.querySelector(target);           // mostly canvas or #canvas
    if (domElement==null) domElement = document.querySelector("[id=" + target + "]"); // this was needed for a DIV element that has an emscripten_set_*_callback_*() on it
// EPIC EDIT end -- nick.shin 2020-06-15 -- 1.39.18
    return domElement;
  },
